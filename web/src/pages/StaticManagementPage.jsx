...
export default function StaticManagementPage() {
  ...
  const presets = {
    income: [
      {
        name: t.presetOpintotuki,
        amount: 27900,
      },
      {
        name: t.presetAsumislisa,
        amount: null,
      },
    ],
    expense: [
  ...
  const handleRemove = async (id, type) => {
    ...
    setItems((prev) => ({
      incomes:
        type === 'staticIncome'
          ? prev.incomes.filter((i) => i.id !== id)
          : prev.incomes,

      expenses:
        type === 'staticExpense'
          ? prev.expenses.filter((i) => i.id !== id)
          : prev.expenses,
    }))
  }
        ...
        <div className="panel">
          ...
          <div className="simple-list">
            {items.incomes.map((item) => (
              <div
                key={item.id}
                className="list-card income-border"
              >
            ...
            {items.incomes.length === 0 && (
              <p className="muted-text">
                {t.noStaticIncomesYet}
              </p>
            )}
          </div>

          <h3 style={{ marginTop: 24 }}>
            {t.staticExpensesTitle}
          </h3>
          ...
          <div className="simple-list">
            {items.expenses.map((item) => (
              <div
                key={item.id}
                className="list-card expense-border"
              >
                <div className="row-between">
                  <strong>{item.name}</strong>
